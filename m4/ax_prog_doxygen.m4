...
#   warranty.
#
#   NOTE: heavily customised for the repository needs
...
# Analogue of DX_REQUIRE_PROG for doxygen
AC_DEFUN([DX_REQUIRE_DOXYGEN], [
AC_MSG_CHECKING([for doxygen support])
AC_ARG_WITH([doxygen],
						[AS_HELP_STRING([--with-doxygen], [use doxygen at path])],
						[with_doxygen=$withval],
						[with_doxygen=no])
AC_MSG_RESULT([$with_doxygen])
if test "x$with_doxygen" != "xno" ; then
AC_SUBST([DX_DOXYGEN], [$with_doxygen])
else
AC_PATH_TOOL([DX_DOXYGEN], [doxygen])
fi
echo "Doxygen at ${DX_DOXYGEN}"
if test "$DX_FLAG_[]DX_CURRENT_FEATUREDX_DOXYGEN" = 1; then
    AC_MSG_WARN([doxygen not found - will not DX_CURRENT_DESCRIPTION])
    AC_SUBST(DX_FLAG_[]DX_CURRENT_FEATURE, 0)
fi
])
...
DX_ARG_ABLE(doc, [generate any doxygen documentation],
            [],
            [],
            [DX_REQUIRE_DOXYGEN([DX_DOXYGEN], doxygen)
...
[[\$(DX_DOCDIR]DX_i[)/\$(PACKAGE).pdf: \$(DX_DOCDIR]DX_i[)/\$(PACKAGE).tag
	\$(DX_V_LATEX)cd \$(DX_DOCDIR]DX_i[)/latex; \\
	rm -f *.aux *.toc *.idx *.ind *.ilg *.log *.out; \\
	\$(DX_PDFLATEX) --halt-on-error refman.tex >> pdflatex-out.log ; \\
	\$(DX_MAKEINDEX) refman.idx; \\
	\$(DX_PDFLATEX) --halt-on-error refman.tex >> pdflatex-out.log ; \\
	countdown=5; \\
	while \$(DX_EGREP) 'Rerun (LaTeX|to get cross-references right)' \\
...
[[\$(DX_DOCDIR]DX_i[)/\$(PACKAGE).tag: \$(DX_CONFIG]DX_i[) \$(pkginclude_HEADERS)
	...
	\$(A""M_V_at)echo Timestamp > \$(DX_DOCDIR]DX_i[)/\$(PACKAGE).tag
