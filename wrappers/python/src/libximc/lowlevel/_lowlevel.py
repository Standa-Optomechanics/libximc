...
def _load_lib():
    ...
    if os_kind == "windows":
        ...
        if cpu_kind == "arm":
            cpu_paths = [os.path.join(cwd, "../library-files/debian-armhf"), os.path.join(ximc_root_path, "debian-armhf")]
        elif cpu_kind == "i386":
            cpu_paths = [os.path.join(cwd, "../library-files/debian-i386"), os.path.join(ximc_root_path, "debian-i386")]
        else:
    ...
    for path in paths:
        try:
            lib = load_from_directory(libs, path)
...
class FeedbackFlags:
    ...
    FEEDBACK_ENC_FILTER_NONE          = 0x00
    FEEDBACK_ENC_FILTER_WEAK          = 0x10
    FEEDBACK_ENC_FILTER_MEDIUM        = 0x20
    FEEDBACK_ENC_FILTER_STRONG        = 0x30
    FEEDBACK_ENC_FILTER_BITS          = 0x30
...
class network_settings_t(Structure):
    _fields_ = [
        ("DHCPEnabled", c_uint),
        ("IPv4Address", c_uint * 4),
        ("SubnetMask", c_uint * 4),
        ("DefaultGateway", c_uint * 4),
    ]

class password_settings_t(Structure):
    _fields_ = [
        ("UserPassword", c_char * 20),
    ]
